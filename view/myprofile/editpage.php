...
<div class="well">				
	<label for="inputPassword" class="control-label text-capitalize"><i class="fa fa-unlock"></i> Please input your current password to proceed</label>
	<input type="password"  class="form-control passwordVerify"  name="passwordVerify">
	</br>
	<a class="btn btn-default proceedverify" href="#" role="button">Proceed</a>
</div>
<script>
$(function() {
	$(".myprofileRow").hide();
	
	$( ".proceedverify" ).click(function() {
		curpass = "<?php echo $_SESSION['current_password']; ?>";
		inputpassuser = $(".passwordVerify").val();
		if(curpass == inputpassuser){
			$(".myprofileRow").show();
			$(".well").hide();
				
		}else{
			alert('Sorry, The password that you have enter is incorrect.')
		}
	...
	<div class="col-lg-12">
		<h2>EDIT <?php echo $user->getusername(); ?> </h2>
			...
			<form role="form" method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>" data-toggle="validator">		
				...
				<div class="form-group" <?php if($_SESSION['role_id'] != 6){ echo 'style="display:none;'; } ?>>
